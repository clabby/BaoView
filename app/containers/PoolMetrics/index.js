        ...
        {ethereum.status !== 'connected' ? (
          ...
          <>
            <Alert variant="warning">
              ...
              <br />
              This resource is for educational purposes only. Please do not use
              this data for financial decisions as there may be discrepancies.
            </Alert>
            <AlertInfo>
              ...
              <br />
              <b>
                ((bao_price_usd * rewards_per_block * blocks_per_year) /
                (total_value_locked_usd)) * 100.0
              </b>
              <hr />
              <b>bao_price_usd</b> is provided by CoinGecko{', '}
              <b>rewards_per_block</b> is received from the master farmer{', '}
              <b>blocks_per_year</b> is constant at <code>6311390</code> for{' '}
              xDai, and <b>total_value_locked_usd</b> is received from the LP
              contract.
            </AlertInfo>
            <h1 className="mt-4 mb-4">
              <div
                className={`pool-icon ${farm.icon.split('/')[1].split('.')[0]}`}
              />
              <PoolTitle>
                {farm.name}
                <br />
                <PoolSubtitle>
                  <Badge variant="secondary">{farm.id}</Badge>
                </PoolSubtitle>
                <br />
                <PoolSubtitle>
                  Pool Metrics <FontAwesomeIcon icon={['fas', 'chart-line']} />
                </PoolSubtitle>
              </PoolTitle>
            </h1>
            ...
            <hr />
            <CandleStickChart
              title="Bao.cx yield per day, per $1000 staked"
              pid={pid}
            />
          </>
        )}
